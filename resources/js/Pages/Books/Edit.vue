...
// Recebemos o livro e a lista de categorias vinda do Controller 
const props = defineProps({
    book: Object,
    categories: Array,
});

// Iniciamos o formulário com todos os campos exigidos pelo PDF 
const form = useForm({
...
const submit = () => {
    // Rota de atualização usando o método PUT [cite: 23]
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="form.isbn" type="text" class="w-full border-gray-300 rounded-md shadow-sm" required>
                            </div>
                        ...
                        <div>
                            ...
                            <input v-model="form.publication_date" type="date" class="w-full border-gray-300 rounded-md shadow-sm" required>
                        </div>

                        <div class="flex items-center justify-end mt-4">
                            <Link :href="route('books.index')" class="text-sm text-gray-600 underline mr-4">
                                Cancelar
                            </Link>
                            <button type="submit" :disabled="form.processing" class="bg-indigo-600 text-white px-4 py-2 rounded-md font-bold hover:bg-indigo-700 transition">
                                {{ form.processing ? 'Salvando...' : 'Atualizar Livro' }}
                            </button>
                        </div>
                    </form>
                </div>
